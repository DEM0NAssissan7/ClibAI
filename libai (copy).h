...
void netcost(Network *network, float *targets){
    ...
    for(int i = 0; i < network->layout[network->out_index]; i++)
        network->cost += square(network->layers[network->out_index][i].value - targets[i]);
    network->average_cost = (network->cost + network->epoch * network->average_cost) / (network->epoch + 1);
}
